...
// ─── Новый эндпоинт: приём готовых строк батчами от браузера ─────────────────
app.post('/api/excel/import-rows', async (req, res) => {
  try {
    ...
    // При первом батче — строим карту старых данных для мержа
    if (isFirst) {
      db._importBuffer = []
      db._importOldMap = {}
      ;(db.rows || []).forEach(r => { if (r.id) db._importOldMap[r.id] = r })
    }

    const EDITABLE = ['assignee', 'controller', 'comment', 'distributedAt', '_history', 'stage', 'archived']
    const oldMap = db._importOldMap || {}

    const merged = newBatch.map(nr => {
      const old = oldMap[nr.id]
      if (old) {
        const m = { ...nr }
        EDITABLE.forEach(f => { if (old[f] !== undefined) m[f] = old[f] })
        m.archived = false
        if (!m.stage) m.stage = getInitialStage(nr.status)
        return m
      }
      return { ...nr, archived: false, stage: getInitialStage(nr.status) }
    })

    if (!db._importBuffer) db._importBuffer = []
    db._importBuffer.push(...merged)

    // Последний батч — финализируем
    const receivedSoFar = db._importBuffer.length
    const isDone = receivedSoFar >= (totalRows || receivedSoFar)

    if (isDone) {
      const newIds = new Set(db._importBuffer.map(r => r.id))
      // Архивируем заявки которых нет в новом файле
      ;(db.rows || []).forEach(oldRow => {
        if (oldRow.id && !newIds.has(oldRow.id)) {
          db._importBuffer.push(oldRow.archived ? oldRow : { ...oldRow, archived: true })
        }
      })
      db.rows = db._importBuffer
      db.importedFrom = name
      db.importedAt = new Date().toISOString()
      delete db._importBuffer
      delete db._importOldMap
      await writeDB(db)
      console.log('Client-import done:', db.rows.length, 'rows')
    } else {
      await writeDB(db)
    }

    res.json({ success: true, received: receivedSoFar, done: isDone })
  } catch(e) {
